...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){
    ...
    syscall_numarg[DSOS_CALL_OPEN_RESOURCE]     = 4;
    ...
    /** MSG QUEUE SYSCALLS INSALLATION **/
...
int disastrOS_openResource(const char *namePtr, int resource_id, int type, int mode) {
    return disastrOS_syscall(DSOS_CALL_OPEN_RESOURCE, namePtr, resource_id, type, mode);
}
...
// Prende in input il nome della coda di messaggi da aprire
int disastrOS_msgQueueOpen(const char *name) {
    return disastrOS_syscall(DSOS_CALL_MQ_OPEN, name);
}

// Chiude il descrittore mqdes associato alla coda di messaggi
int disastrOS_msgQueueClose(int mqdes) {
    return disastrOS_syscall(DSOS_CALL_MQ_CLOSE, mqdes);
}

/* Prende in input il nome della coda di messaggi da rimuovere immediatamente.
...
/* Rimuove il messaggio con priorità più alta dalla coda di messaggi a cui si riferisce mqdes
 * e lo inserisce nel buffer a cui punta msg_ptr. Viene specificata la dimensione del buffer
 * puntato da msg_ptr. */
...
/* Aggiunge il messaggio puntato da msg_ptr alla coda di messaggi a cui si riferisce mqdes.
 * Viene specificata la lunghezza del messaggio puntato da msg_ptr e la priorità del messaggio. */
