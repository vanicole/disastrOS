...
/*
    if (!desc) {
        printf("[ERROR] The message queue with fd %d isn't opened for the running process!\n", mqdes);
...
*/
    ...
    printf(">> Allocating the message in the subqueue with priority %u\n", priority);
    Message *msg = Message_alloc(msg_ptr, msg_len);

    if (!msg) {
        printf("[ERROR] Failed to allocate the new message!\n");
        running->syscall_retvalue = DSOS_EMQ_WRITE;
        return;
    }
    // inserisco (scrivo) messaggio nella lista di messaggi della sottocoda con certa priorità
